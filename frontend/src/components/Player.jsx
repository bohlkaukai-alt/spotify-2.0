    ...
    <div className="h-[72px] bg-spotify-black border-t border-[#282828] flex items-center px-4 z-50 shrink-0">
      ...
      <div className="flex items-center gap-3 w-[30%] min-w-[180px]">
        ...
        <div className="min-w-0">
          <p className="text-sm font-medium truncate hover:underline cursor-pointer">{currentTrack?.title || 'Kein Song'}</p>
          <p className="text-xs text-spotify-text truncate hover:underline cursor-pointer">{currentTrack?.artist || '—'}</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center max-w-[722px] gap-1">
        ...
        <div className="flex items-center gap-2 w-full">
          <span className="text-[11px] text-spotify-text w-10 text-right tabular-nums">{formatTime(progress)}</span>
          <input type="range" min="0" max={duration || 0} value={progress}
            ...
            className="flex-1" />
          <span className="text-[11px] text-spotify-text w-10 tabular-nums">-{formatTime(remaining > 0 ? remaining : 0)}</span>
        </div>
      </div>

      {/* Volume + Fullscreen */}
      <div className="w-[30%] flex justify-end items-center gap-3 min-w-[180px]">
        <button onClick={onFullscreen} className="text-spotify-text hover:text-white transition-colors" title="Vollbild">
          <Maximize2 size={16} />
        </button>
