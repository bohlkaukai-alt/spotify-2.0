...
export default function TrackList({ tracks, title }) {
  ...
  const toggleFavorite = async (track) => {
    if (favorites.has(track.id)) {
      ...
      setFavorites((prev) => { const next = new Set(prev); next.delete(track.id); return next; });
    } else {
      await db.favorites.add({
        trackId: track.id, title: track.title, artist: track.artist,
        thumbnail: track.thumbnail, addedAt: new Date(),
      });
    ...
    <div className="px-6 pb-24">
      ...
      {tracks.map((track, i) => (
        <div key={track.id}
          ...
          <button className="hidden group-hover:block text-white" onClick={() => handlePlay(track)}>
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-spotify-text truncate hover:underline cursor-pointer hover:text-white"
          ...
          <span className="text-sm text-spotify-text truncate">{track.album || '—'}</span>
          <span className="text-sm text-spotify-text text-right tabular-nums">{formatDuration(track.duration)}</span>

          <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={(e) => { e.stopPropagation(); toggleFavorite(track); }}
              className={favorites.has(track.id) ? 'text-spotify-green' : 'text-spotify-text hover:text-white'} title="Favorit">
            ...
            <button onClick={(e) => { e.stopPropagation(); usePlayerStore.getState().addToQueue(track); }}
              className="text-spotify-text hover:text-white" title="Zur Queue">
              <Plus size={14} />
            </button>
            <button onClick={(e) => { e.stopPropagation(); downloadTrack(track); }}
              className="text-spotify-text hover:text-white" title="Download">
              <Download size={14} />
            </button>
          </div>
        </div>
